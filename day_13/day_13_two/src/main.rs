...
impl Area {
    ...
    fn mirror(nums: &Vec<u32>) -> u32 {
        ...
        for i in 1..length {
            ...
            let mut smudge = detect_smudge(nums[i - 1], nums[i]);
            if smudge != 0 {
                found = true;
            };
            if smudge != 0 || nums[i - 1] == nums[i] {
                nums[i - 1] = nums[i - 1] ^ smudge;
                smudge = 0;
                let mut left = i - 1;
                let mut right = i;
                let split = right;
                while left != usize::MAX && right < length {
                    if !found {
                        smudge = detect_smudge(nums[left], nums[right]);
                        if smudge != 0 {
                            println!("Located between index {} and {}", left, right);
                            found = true
                        };
                    }
                    if nums[left] == nums[right] || smudge != 0 {
                        nums[left] = nums[left] ^ smudge;
                        smudge = 0;
                ...
                if (left == usize::MAX || right == length) && found {
                    println!("Found mirror at: {}", split);
                    return split as u32;
                }
                println!("Nevermind, that wasn't a mirror.");
            }
            found = false;
        }
        println!("No mirror.");
...
fn detect_smudge(num1: u32, num2: u32) -> u32 {
    let mut diff = num1 ^ num2;
    if (diff != 0) && (diff & (diff - 1) == 0) {
        println!("Smudge found with: {} and {}", num1, num2);
        let mut counter = 0;
        while diff != 1 {
            diff /= 2;
            counter += 1;
        }
        return 2_u32.pow(counter);
    } else {
        return 0;
    }
}
